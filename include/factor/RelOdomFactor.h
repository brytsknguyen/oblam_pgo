...
class RelOdomFactor : public ceres::SizedCostFunction<6, 7, 7>
{
  ...
  public:
          ...
          p_odom_n(_p_odom_n), q_odom_n(_q_odom_n)
    {
      ...
      Eigen::Matrix<double, 3, 3> DeltaRot_inv   = DeltaRot.transpose();
      Eigen::Matrix<double, 3, 3> DeltaPos_sksym = Util::skewSymmetric(Roti_inv*(p_odom_j - p_odom_i));
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (jacobians)
        {
            ...
            if(jacobians[1])
            {
                ...
                jacobian_pose_j.block<3, 3>(3, 3) = Util::Qleft(delta_q.inverse()
                                                                    * Qi.inverse() * Qj
                                                                  ).bottomRightCorner<3, 3>();
                ...
                {
                    printf("numerical unstable in vio factor jacobian. dt: %f\n");
                    // std::cout << pre_integration->jacobian << std::endl;
                    // ROS_BREAK();
                }
